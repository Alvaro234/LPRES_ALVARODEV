...
- Caida de presion con coeficiente landa entre el nodo N-1 (final del grano) hasta la tobera N ¿Modulo a parte?
...
EQUAL							 REAL Pout				      UNITS u_m				RANGE ZERO,Inf			
EQUAL							 REAL Pin				      UNITS u_m				RANGE ZERO,Inf		
				
REAL rp0_intermedia				
				
CONTINUOUS

rp0_intermedia = (1.27955E-05)* (0.5*(Pin+Pout))**0.4
...
---------------------------------------------------------------------------INTERFACES (De 1 a N+1)---------------------------------------------------------------------
		...
		S[nodos+1]' =	2*MATH.PI*rp0[nodos]
		Ap[nodos+1]' = S[nodos+1]*rp0[nodos]
...
---------------------------------------------------------------------- VOLUMEN DISCRETO (De 1 a N)--------------------------------------------------------------
		...
		dg[i] = Rho_P*rp0[i]*S[i]*dx 			-- ¿Añadir terminos de masa de ignicion?
		rp0[i] = Factor_a*P[i]**Exponente            
		END EXPAND_BLOCK 
		EXPAND_BLOCK(i IN 1,nodos-1)  
		P[i+1] = P[i]-((g[i])*(U[i+1]-U[i]) + dg[i]*U[i+1])/Ap[i+1]	-- Revisar dejar solo en funcion de la interfaz		------- Lo mismo para U y T
		Tt[i+1] = (g[i]*Tt[i]+dg[i]*Tc)/(g[i+1])	
		END EXPAND_BLOCK
		EXPAND_BLOCK(i IN 1,nodos) -- Variables de remanso -- ¿Hacer las velocidades como la media entre interfaces? 
		T[i] = Tt[i]/(1+0.5*(gamma-1)*MACH[i]**2)
		pt[i] = P[i]*(Tt[i]/T[i])**(gamma/(gamma-1)) 
		SoundSpeed[i]= sqrt(gamma*R_gas*T[i])
		MACH[i]= 0.5*(U[i]+U[i+1])/SoundSpeed[i]
		Rho[i] = P[i] /(R_gas*T[i])
		END EXPAND_BLOCK			
